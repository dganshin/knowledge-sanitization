...
def train(
    ...
    prompter = Prompter(prompt_template_name, template_dir)
    ...
    def load_adapter_lora(model_id, **kwargs):
    ...
    def check_weights_zero(lora_weights):
        for w_name, w_value in lora_weights.items():
            if torch.all(w_value == 0.0):
                raise ValueError(f"weights are zero: {w_name}")

    check_weights_zero(load_adapter_lora(output_dir))
