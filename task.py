...
def main():
    ...
    progress = tqdm(total=len(batch_records))
    for batch in chunked(batch_records, args.eval_batch_size):
        prompts = [record["full_prompt"] for record in batch]
        ...
        for record, generated_text in zip(batch, decoded_outputs):
            ...
            correct = calc_exact_match(predicted_answer, record["task_output"]) # 0/1
            n_correct += correct
            total += 1
            ...
            if total % args.log_interval == 0:
                ...
                progress.set_postfix({"acc": f"{accuracy:.4f}", "done": total})
